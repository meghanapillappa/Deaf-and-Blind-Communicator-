...
def display_sign_language(text, output_widget):
    # Clear old images
    for widget in output_widget.winfo_children():
        widget.destroy()
    ...
    for letter in text.upper():
        ...
        if os.path.exists(image_path):
            ...
            img = img.resize((50, 50), Image.LANCZOS)  # Fixed (no ANTIALIAS)
            img = ImageTk.PhotoImage(img)
            label = tk.Label(output_widget, image=img, bg="#B3E5FC")
...
def open_conversation():
    ...
    tk.Label(
        conversation_window, text=welcome_message, bg="#D1C4E9", fg="blue",
        wraplength=400, font=("Arial", 12, "bold")
    ).pack()
    ...
    # Deaf/Mute user input
    tk.Label(
        conversation_window, text="Deaf/Mute User: Type below and press Send",
        bg="#D1C4E9", fg="purple", font=("Arial", 10, "bold")
    ).pack()
    conversation_input = tk.Entry(conversation_window, width=50)
    conversation_input.pack()
    tk.Button(
        conversation_window, text="Send", bg="#64B5F6", fg="white",
        font=("Arial", 10, "bold"),
        command=lambda: send_conversation_text(conversation_input, conversation_output)
    ).pack()

    # Blind user speech
    global output_label
    output_label = tk.Label(
        conversation_window, text="Blind User: Speak into the microphone",
        bg="#D1C4E9", fg="black", font=("Arial", 10, "bold")
    )
    output_label.pack()
    tk.Button(
        conversation_window, text="Listen", bg="#FF8A65", fg="white",
        font=("Arial", 10, "bold"),
        command=lambda: threading.Thread(
            target=detect_speech_with_opencv, args=(conversation_output,)
        ).start()
    ).pack()

    # Conversation output
    tk.Label(
        conversation_window, text="Conversation:", bg="#D1C4E9",
        fg="dark green", font=("Arial", 12, "bold")
    ).pack()
    conversation_output = tk.Text(
        conversation_window, height=10, width=50,
        bg="white", fg="black", font=("Arial", 10)
    )
    conversation_output.pack()

    tk.Button(
        conversation_window, text="Reset Conversation", bg="#F06292", fg="white",
        font=("Arial", 10, "bold"),
        command=lambda: reset_conversation(conversation_output)
    ).pack()
...
welcome_message = (
    "Welcome to the communication system. Deaf or mute users can type, "
    "and blind users can speak.\nBeing different doesn’t mean being disconnected—"
    "welcome to a world where everyone is heard!"
)

tk.Label(
    root, text=welcome_message, bg="#B3E5FC", fg="blue",
    wraplength=400, font=("Arial", 12, "bold")
).pack()
...
if os.path.exists(image_path):
    ...
    root.image = img  # Keep reference

root.after(1000, lambda: speak_text(welcome_message))

# Deaf/Mute text input in main window
tk.Label(root, text="Deaf/Mute User: Type and press Send", bg="#B3E5FC", fg="purple", font=("Arial", 10, "bold")).pack()
text_input = tk.Entry(root, width=40)
text_input.pack()
tk.Button(root, text="Send", bg="#64B5F6", fg="white", font=("Arial", 10, "bold"), command=send_text).pack()

# Text output (conversation log in main window)
text_output = tk.Text(root, height=8, width=50, bg="white", fg="black", font=("Arial", 10))
text_output.pack()
...
# Button to open full conversation window
